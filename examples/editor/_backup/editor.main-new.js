...
<!--<script src="./js/babel/babel-standalone@7.12.4.js" type="text/javascript"></script>-->
...
class Editor {
    ...
    createApp(editorData) {
        // let scriptBlocks = JQUERY + BOOTSTRAP_CSS + BOOTSTRAP_JS + REQUIRE_METHOD + BABEL_CORE + BABEL_PRESET_ENV;
        let scriptBlocks = JQUERY + BOOTSTRAP_CSS + BOOTSTRAP_JS + REQUIRE_METHOD + BABEL_CORE;
    ...
    renderOutput(editorData) {
        ...
        // preview.body.innerHTML = editorData;
        this.preview.close();
        // this.displayConsole(editorData);
        clearInterval(this.timer);
    }
    ...
    displayConsole(editorData) {
        if (window.console) {
            console.log = function () {
                var output = '',
                    outputElem = document.getElementById('console');
                for (var i = 0; i < arguments.length; i++) {
                    output += arguments[i] + ' ';
                }
                var test = eval(editorData);

                // console.innerText+=output+"\n";
                outputElem.innerHTML = test;
            }
